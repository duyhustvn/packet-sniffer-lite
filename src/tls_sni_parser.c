...
static bool get_u16(const uint8_t *buf, size_t len, size_t *off,
                    uint16_t *out) {
  if (*off + 2 > len) {
    return false;
  }
...
// - handshake_len la do dai ClientHello body, KHONG tinh 4 byte handshake
// header.
//
...
bool extract_tls_sni(const uint8_t *payload, size_t payload_len, char *host,
  ...
  if (!get_u8(payload, record_end, &off, &handshake_type) ||
      handshake_type != 1) {
    return false;
  }
  ...
  size_t handshake_len = ((size_t)payload[off] << 16) |
                         ((size_t)payload[off + 1] << 8) | payload[off + 2];
  off += 3;
  ...
  if (!skip_bytes(handshake_end, &off, 2) ||
      !skip_bytes(handshake_end, &off, 32)) {
    return false;
  }
