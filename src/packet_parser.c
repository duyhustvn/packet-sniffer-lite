...
static bool read_u16(const uint8_t *buf, size_t len, size_t off,
                     uint16_t *out) {
...
//   The TCP header is not necessarily at packet[0]; its position is given by
//   tcp_off.
...
// Tong quan:
// Parse mot Ethernet frame da capture va chi chap nhan packet TCP tren
// IPv4/IPv6. Ham nay bo qua Ethernet header, xu ly them VLAN tag neu co, sau do
// dua phan IP packet cho parse_ipv4() hoac parse_ipv6(). Neu packet khong phai
// IPv4/IPv6, khong phai TCP, bi cat ngan, fragment, hoac format khong hop le
// thi tra ve false.
//
// Arguments:
...
// - out: noi ham ghi ket qua parse duoc. Ham se reset struct nay ve 0 truoc khi
// doc.
//
...
// - out->payload / out->payload_len: con tro va do dai phan data sau TCP
// header.
//
// Vi du:
// Neu frame la Ethernet + IPv4 + TCP + HTTP request, ham tra ve true va
